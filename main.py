...
def main():
	...
	parser.add_argument('--train_data', type=str, required=True, help='Training text dataset.')
	parser.add_argument('--evaluation_data', type=str, required=True, help='Evaluation text dataset.')
	parser.add_argument('--train_tags', type=str, required=True, help='Training tags dataset.')
	parser.add_argument('--evaluation_tags', type=str, required=True, help='Evaluation tags dataset.')
	...
	if args.do_train():
		...
		with open(args.checkpoint + '/settings.bin', 'wb') as f:
		    torch.save([word_vocab, word2idx, idx2word, tags_vocab, tag2idx, idx2tag, msl, 
		    			args.embedding_dim, args.hidden_dim, args.dropout, args.bidirectional], f)

	if args.do_predict:
		# Load the vocabularies
		with open(args.checkpoint + '/settings.bin', 'rb') as f:
		    word_vocab, word2idx, idx2word, tags_vocab, tag2idx, idx2tag, msl, embedding_dim, hidden_dim, dropout, bidirectional = torch.load(f)
